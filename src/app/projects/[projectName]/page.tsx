...
import { CaseStudyIcon, ChromeWebStoreIcon, FirefoxIcon, LiveDemoIcon, SourceCodeIcon } from "@/components/ui/icons";
...
export default async function ProjectPage({ params }: ProjectRouteProps) {
    ...
    return (
        <main className="flex flex-col gap-[50px] justify-start bg-bg min-h-screen px-mobile-scr-mg-x py-25 pt-35 sm:px-scr-mg-x">
            <BackButton />
            <section className="px-4 py-8 rounded-2xl bg-overlay shadow-sm flex flex-col gap-4 sm:max-w-[800px] sm:gap-8 sm:px-10 sm:py-20 sm:rounded-4xl self-center" id="project-view">
                <h2 className="text-2xl font-bold text-txt sm:text-4xl">
                    Project: {project.title}
                </h2>
                <p className="font-normal text-base text-txt/80 normal-case tracking-normal sm:text-2xl">{project.shortExcerpt}</p>
                <ScreenshotCarousel projectData={project} />
                <ProgressStatus variant={project.status} isHome={false} />
                <div className="flex flex-row flex-wrap gap-2">
                    {project.liveDemoLink &&
                        <CustomButton buttonText="Live Demo" buttonLink={project.liveDemoLink} ButtonIcon={LiveDemoIcon} />
                    }
                    {project.extensionLink &&
                        <CustomButton buttonText="Chrome Web Store" buttonLink={project.extensionLink} ButtonIcon={ChromeWebStoreIcon} />
                    }
                    {project.firefoxAddonLink &&
                        <CustomButton buttonText="Firefox Add-on" buttonLink={project.firefoxAddonLink} ButtonIcon={FirefoxIcon} />
                    }
                    {project.githubLink &&
                        <CustomButton buttonText="Source Code" buttonLink={project.githubLink} ButtonIcon={SourceCodeIcon} />
                    }
                    {project.readmeSrc &&
                        <CustomButton buttonText="Case Study" buttonLink="#case-study" ButtonIcon={CaseStudyIcon} />
                    }
                </div>
                <section id="case-study" className="prose prose-invert max-w-none">
                    <ReactMarkdown
                        ...
                        components={{
                            ...
                            p: ({ children }) => (
                                <p className="text-base text-txt/80 normal-case tracking-normal leading-relaxed mb-2">{children}</p>
                            ),
                            a: ({ children, href }) => (
                                <Link target="_blank" className="underline decoration-1.5 underline-offset-2" href={href as string}>{children}</Link>
                            ),
                            ul: ({ children }) => (
                                <ul className="list-disc list-inside">{children}</ul>
                            ),
                            li: ({ children }) => (
                                <li className="text-base text-txt/80 normal-case tracking-normal leading-relaxed">{children}</li>
                            ),
                            table: ({ children }) => (
